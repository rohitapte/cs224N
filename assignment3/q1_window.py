...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for word_pos,word in enumerate(sentence):
            pad_left=[]
            pad_right=[]
            left_index=word_pos-window_size
            if left_index<0:
                pad_left=[start for i in range(-left_index)]
                left_index=0
            right_index=word_pos+window_size+1
            if right_index>sentence_length:
                pad_right=[end for i in range(right_index-sentence_length)]
                right_index=sentence_length
            padded_word=pad_left+sentence[left_index:right_index]+pad_right
            item_to_add=[j for i in padded_word for j in i]
            windowed_data.append((item_to_add,labels[word_pos]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder=tf.placeholder(tf.int32,shape=[None,Config.n_window_features],name='input')
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        vocab=tf.Variable(self.pretrained_embeddings)
    ...
    def add_prediction_op(self):
        ...
        W_shape = (Config.n_window_features*Config.embed_size,Config.hidden_size)
        b1_shape = (1, Config.hidden_size)
        U_shape = (Config.hidden_size, Config.n_classes)
        b2_shape = (1, Config.n_classes)
        W=tf.get_variable(name='W',shape=W_shape,dtype=tf.float32,initializer=xavier_initializer)
        b1=tf.Variable(tf.zeros(b1_shape), name='b1')
        U=tf.get_variable(name='U',shape=U_shape,dtype=tf.float32,initializer=xavier_initializer)
        b2=tf.Variable(tf.zeros(b2_shape), name='b2')
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred,labels=self.labels_placeholder),name='cross_entropy_loss')
        ### END YOUR CODE
        return loss

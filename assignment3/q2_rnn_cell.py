...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_h=tf.get_variable(name='W_h',shape=W_h_shape,dtype=tf.float32,initializer=xavier_initializer)
            W_x=tf.get_variable(name='W_x', shape=W_x_shape,dtype=tf.float32,initializer=xavier_initializer)
            b=tf.get_variable("b",b_shape,initializer=constant_initializer)

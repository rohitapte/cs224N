...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sentence_truncated=sentence[:max_length]
        labels_truncated=labels[:max_length]
        sentence_length=len(sentence_truncated)
        masking_seqeunce=[True for i in range(sentence_length)]
        if sentence_length<max_length:
            sentence_truncated.extend([zero_vector for i in range(max_length-sentence_length)])
            labels_truncated.extend([zero_label for i in range(max_length-sentence_length)])
            masking_seqeunce.extend([False for i in range(max_length-sentence_length)])
        ret.append((sentence_truncated,labels_truncated,masking_seqeunce))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        h_shape=(1,Config.hidden_size)
        b_2_shape=(Config.n_classes,)
        U=tf.get_variable(name='U',shape=U_shape,dtype=tf.float32,initializer=xavier_initializer)
        b_2=tf.get_variable("b_2",b_2_shape,initializer=constant_initializer)
        h=tf.zeros(shape=h_shape,name='h')
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                y_t=tf.add(tf.matmul(o_drop_t,U),b_2)
                preds.append(y_t)
                ### END YOUR CODE
        ...
        #pred: tf.Tensor of shape (batch_size, max_length, n_classes)
        preds_as_tensor=tf.stack(preds,axis=0)
        #preds_as_tensor is (max_len,batch_size,n_classes) so reshape to 1,0,2
        preds=tf.transpose(preds_as_tensor,[1,0,2])
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        masked_values=tf.boolean_mask(cross_entropy,self.mask_placeholder)
        loss=tf.reduce_mean(masked_values)
        ### END YOUR CODE
        return loss

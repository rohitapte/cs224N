...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z_shape=(self.input_size,self.state_size)
            U_z_shape=(self.state_size,self.state_size)
            b_z_shape=(self.state_size,)
            W_r_shape=(self.input_size,self.state_size)
            U_r_shape=(self.state_size, self.state_size)
            b_r_shape=(self.state_size,)
            W_o_shape=(self.input_size,self.state_size)
            U_o_shape=(self.state_size,self.state_size)
            b_o_shape=(self.state_size,)
            W_r=tf.get_variable(name='W_r',shape=W_r_shape,dtype=tf.float32,initializer=xavier_initializer)
            U_r=tf.get_variable(name='U_r',shape=U_r_shape,dtype=tf.float32,initializer=xavier_initializer)
            b_r=tf.get_variable(name='b_r',shape=b_r_shape,dtype=tf.float32,initializer=constant_initializer)
            W_z=tf.get_variable(name='W_z',shape=W_z_shape,dtype=tf.float32,initializer=xavier_initializer)
            U_z=tf.get_variable(name='U_z',shape=U_z_shape,dtype=tf.float32,initializer=xavier_initializer)
            b_z=tf.get_variable(name='b_z',shape=b_z_shape,dtype=tf.float32,initializer=constant_initializer)
            W_o=tf.get_variable(name='W_o',shape=W_o_shape,dtype=tf.float32,initializer=xavier_initializer)
            U_o=tf.get_variable(name='U_o',shape=U_o_shape,dtype=tf.float32,initializer=xavier_initializer)
            b_o=tf.get_variable(name='b_o',shape=b_o_shape,dtype=tf.float32,initializer=constant_initializer)
            ...
            o_t=tf.nn.tanh(tf.matmul(inputs,W_o)+tf.multiply(r_t,tf.matmul(state,U_o))+b_o)

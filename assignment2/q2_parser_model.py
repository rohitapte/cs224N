...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        W_shape=(Config.embed_size*Config.n_features,Config.hidden_size)
        b1_shape=(1,Config.hidden_size)
        U_shape=(Config.hidden_size,Config.n_classes)
        b2_shape=(1,Config.n_classes)
        W=tf.Variable(xavier_initializer(W_shape),name='W_variable')
        b1=tf.Variable(tf.zeros(b1_shape),name='b1_variable')
        U=tf.Variable(xavier_initializer(U_shape),name='U_variable')
        b2=tf.Variable(tf.zeros(b2_shape),name='b2_variable')
        h=tf.nn.relu(tf.add(tf.matmul(x,W),b1))
        h_drop=tf.nn.dropout(h,1-self.dropout_placeholder)
        if Config.extensions_on==True:
            W_second_shape=(Config.hidden_size,Config.hidden_size)
            b_second_shape=(1,Config.hidden_size)
            W_second=tf.Variable(xavier_initializer(W_second_shape), name='W_second')
            b_second=tf.Variable(tf.zeros(b_second_shape), name='b_second')
            h_second=tf.nn.relu(tf.add(tf.matmul(h_drop,W_second),b_second))
            h_second_drop = tf.nn.dropout(h_second, 1 - self.dropout_placeholder)
            pred=tf.add(tf.matmul(h_second_drop,U),b2)
        else:
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        loss=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred,labels=self.labels_placeholder),name='cross_entropy_loss')
        ### END YOUR CODE
        return loss
